...
def load_and_cache_gen_data(args, filename, pool, tokenizer, split_tag, only_src=False, is_sample=False):
    ...
    cache_fn = f"{args.cache_path}/{split_tag + ('_src' if only_src else '') + data_tag}.pt"
...
def load_and_cache_clone_data(args, filename, pool, tokenizer, split_tag, is_sample=False):
    cache_fn = f"{args.cache_path}/{f'{split_tag}_all' if args.data_num == -1 else '_%d' % args.data_num}.pt"
    examples = read_examples(filename, args.data_num, args.task)
    if is_sample:
        ...
        for task in task_list:
            ...
            for sub_task in sub_tasks:
                ...
                if only_src:
                    ...
                    data = TensorDataset(all_source_ids, all_target_ids)
                examples_data_dict[
                    f'{task}_{sub_task}' if sub_task != 'none' else task
                ] = (examples, data)
...
def get_filenames(data_root, task, sub_task, split=''):
    if task == 'clone':
        ...
        data_dir = f'{data_root}/{task}'
        if sub_task == 'cs-java':
            train_fn = f'{data_dir}/train.java-cs.txt.cs,{data_dir}/train.java-cs.txt.java'
            dev_fn = f'{data_dir}/valid.java-cs.txt.cs,{data_dir}/valid.java-cs.txt.java'
            test_fn = f'{data_dir}/test.java-cs.txt.cs,{data_dir}/test.java-cs.txt.java'
        else:
            train_fn = f'{data_dir}/train.java-cs.txt.java,{data_dir}/train.java-cs.txt.cs'
            dev_fn = f'{data_dir}/valid.java-cs.txt.java,{data_dir}/valid.java-cs.txt.cs'
            test_fn = f'{data_dir}/test.java-cs.txt.java,{data_dir}/test.java-cs.txt.cs'
    if split == 'dev':
        return dev_fn
    elif split == 'test':
        return test_fn
